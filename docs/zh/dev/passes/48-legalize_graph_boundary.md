...
| ---- | -------------- | -------------- |
| 边界标量在区域内被**派生**出来 | 归类为静态数据，把第一次调用的值冻进录制；隐式取值转换已弃用 | **Step A** —— 把计算外提到调用点；当冻结下来的值可证明就是正确值时，原地保留 |
...
```cpp
const auto batch = args.scalar(0);  // 保留形参来源
int64_t n = batch;                  // 已弃用的普通数值转换
g0_params_t0.add_scalar(n);         // 任务不再继承该形参
```
...
[`ExplicitReturnedParamIndices`](28-normalize_return_order.md) 跟过去，也就是
...
外提所需要的东西由 [`NormalizeReturnOrder`](28-normalize_return_order.md) 提供：它会
