...
| [`img2col`][pypto.language.tile.img2col] | `pl.tile.` | A2/A3 TIMG2COL：将完整的 NZ Mat Tile `[H*W, C]` 展开为供 matmul 使用的 Left Tile `[M, K]`。几何参数使用 `(H, W)`，零填充使用 `(上, 下, 左, 右)`。K 轴顺序为 `(C1, KH, KW, C0)`，其中 `C0=32/sizeof(dtype)`。H*W/M 必须按 16 对齐，C/K/pos_k 必须按 C0 对齐 |
