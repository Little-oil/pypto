...
Decomposes composite tile / distributed ops into primitive operations so codegen does not need to emit their high-level forms. Today the pass handles standalone `tile.pow` / `tile.mean` / `tile.clamp`, `tile.sin` / `tile.cos` (FP32 Cody-Waite + Horner), packed `tile.tquant_mx`, and `pld.tensor.*` distributed collectives (`allreduce` (mesh and ring), `allgather`, `reduce_scatter`, `broadcast`, `barrier`). Mesh and ring allreduce may also create a metadata-preserving `tensor.view` so tile load/remote/store operate on a 2D flattened target window.

## Overview

`LowerCompositeOps` is a function-level pass that rewrites every `var = Call(...)` `AssignStmt` whose callee appears in the pass's lowering dispatch table. The standalone math rules decompose `tile.pow`, `tile.mean`, and `tile.clamp` into primitive arithmetic, reduction, and cast operations. For `tile.sin` / `tile.cos`, the rule emits a fixed-shape primitive tile-op recipe using `tile.muls`, `tile.adds`, `tile.add`, `tile.sub`, `tile.mul`, and `tile.cast`: Cody-Waite range reduction (4-part π split) followed by a degree-9 odd Horner polynomial. `tile.tquant_mx` becomes value-returning `tile.tquant_mx_raw` + `tile.tmov_x2zz` (gather_compare-style SSA) plus explicit workspace tiles; public FP8 aliases are added via `reinterpret_view` / `transpose_view`. For `pld.tensor.*` distributed collectives, the rules emit the cross-rank recipes documented below; `pld.tensor.allreduce` remains explicit-signal in InCore/composite lowering. The original target `Var` is preserved as the LHS of the final `AssignStmt`, so downstream uses keep the same name and identity.
...
The pass is **structural no-op** on programs that contain no registered composite call such as `tile.pow`, `tile.mean`, `tile.clamp`, `tile.sin`, `tile.cos`, `tile.tquant_mx`, or `pld.tensor.*` distributed collectives: every other statement passes through `IRMutator::VisitStmt_`. The decomposition emits only primitive tile ops, the internal value-returning `tile.tquant_mx_raw` / `tile.tmov_x2zz` forms, and distributed primitives, none of which the mutator rewrites — so the pass is also **idempotent**.
...
`LowerCompositeOps` is the **first entry of `tile_pto_passes`** in the `Default` pipeline (see `python/pypto/ir/pass_manager.py`), running immediately after `ConvertTensorToTileOps` (slot 12) and `OptimizeOrchTensors` (slot 13). At this point tensor-level composite calls (`tensor.pow`, `tensor.mean`, `tensor.clamp`, `tensor.sin`, and `tensor.cos`) have been rewritten to their tile equivalents by the conversion registry, and the tile pipeline is about to start tile-shape canonicalisation. Lowering these recipes before `FlattenTileNdTo2D` keeps the decomposition independent of the 2D-flattening rules — every primitive tile op in the recipes has well-defined behaviour at any supported rank. Packed `tile.tquant_mx` is also lowered here (after `FlattenCallExpr` has stabilized tuple consumers).

**Pass order vs memory space:** `LowerCompositeOps` runs **before** `InferTileMemorySpace`. Scratch tiles created by the `tile.tquant_mx` rule therefore stamp `MemorySpace::Vec` explicitly on `tile.create` so later memory planning still sees addresses; this is intentional, not a signal that Infer should have already run.
...
The lowering creates source-dtype `max` / `scaling` write-only workspace tiles, then binds a value-returning `tile.tquant_mx_raw(src, max, scaling)` whose `TupleType{INT8 dst, UINT8 exp}` elements are projected with `TupleGetItem` (same SSA pattern as `tile.gather_compare`). Codegen resolves those projections via `ResolveTupleResultElements` and emits `pto.tquant.mx` with four outs (dst, exp, max, scaling). Public `group_axis` matches PTOAS `grpAxis`: axis1 keeps `[M,K]` and returns scale `[M,K/32]`; axis0 first transposes `[N,K]` to `[K,N]` and returns scale `[K/32,N]`. Both forms then bind value-returning `tile.tmov_x2zz(exp, tmp)`. Axis1 tmp capacity is `64 + ceil(rows/16)*cols` bytes (33-byte-aligned); axis0 uses a minimal 32-byte Vec pad required by `TMovDnTo2Zz`. Axis0 follows pto-isa `TMovDnTo2Zz` (pin `be5ccb76`): DN `[M̂,N]` → ZZ `[N,M̂]` row/row, then a zero-copy `tile.transpose_view` yields the public `[M̂,N]` col/col scale. A zero-copy FP8 data alias and FP8E8M0 scale alias complete the public tuple. Write effects on the workspace args keep the Calls alive under DCE when a public result is unused.
...
Running `LowerCompositeOps` twice produces identical IR after the first run: the recipes emit only primitive tile ops, internal `tile.tquant_mx_raw` / `tile.tmov_x2zz`, and the distributed primitives listed below. None of those results is itself a registered composite call, so the second invocation visits the body and changes nothing.

The `tile.tquant_mx` rule emits the non-composite `tile.tquant_mx_raw` and `tile.tmov_x2zz`, so MX lowering is idempotent as well.
