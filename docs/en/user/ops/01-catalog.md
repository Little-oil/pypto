...
| [`img2col`][pypto.language.tile.img2col] | `pl.tile.` | A2/A3 TIMG2COL: unfold a full NZ Mat tile `[H*W, C]` into a Left tile `[M, K]` for matmul. Geometry uses `(H, W)` pairs; zero padding uses `(top, bottom, left, right)`. K order is `(C1, KH, KW, C0)`, with `C0=32/sizeof(dtype)`. H*W/M require 16 alignment; C/K/pos_k require C0 alignment |
...
| [`cast`][pypto.language.cast] | `pl.` | Convert dtype — may expand to a multi-hop chain, see [LegalizeTileCast](../../dev/passes/17-legalize_tile_cast.md) |
