...
def lower_composite_ops() -> Pass:
    ...
    Today the pass handles standalone ``tile.pow`` / ``tile.mean`` /
    ``tile.clamp``, ``tile.sin`` / ``tile.cos``, packed ``tile.tquant_mx``, and
    explicit-signal InCore ``pld.tensor.allreduce``. Host-level allreduce is
